...
app.get('/getInfoPlayNow', (req, res) => {
    ytdl.getInfo(req.query.id, (err, info) => {
        ...
        let songs = [extractSong(info)];
        let stack = []
        for (let i = 0; i < relatedVideos.length; i++) {
            let id = relatedVideos[i].id || relatedVideos[i].video_id
            stack.push((cb) => {
                let video_id = relatedVideos[i].id || relatedVideos[i].video_id
                ytdl.getInfo(video_id, cb)
            })

            if(relatedVideos.length === stack.length) {
